...
$(document).ready(function() {
      validateForm = $("#busiYbCompanyInfoForm").Validform({
		...
		beforeSubmit:function(curform){
		    ...
		    // var selectRowDatas = getRowDatas("cpyflTbl",true);
			// var str = JSON.stringify(selectRowDatas); 
			// console.log(str);
		    for (var key in selectRowDatas) {
		    	var str1 = JSON.stringify(selectRowDatas[key]);
		    	console.log(str1);
		    	if(str1.indexOf("editable") > 0&&str1.indexOf("inline-edit-cell") ) {
		    		return false;
		    	}
...
//添加扩展数据
function addRowDataExt(gridId, rowid,dataRow){
	var gridObject = $('#' + gridId);  
	gridObject.jqGrid("addRowData", rowid, dataRow, "last"); 
}
...
function delRowDataExt(gridId, rowid){
	var gridObject = $('#' + gridId);  
...
function getRowExtendDatas(gridId, rowid){
	var pageInfo=getGridRowData(gridId,rowid);
	return pageInfo;
}
...
function initGrid(girdid,colModel,data, caption, multiselect, height){
	 //初始化用户列表
 	 $("#"+girdid).jqGrid({
 	         datatype: "local",
			 data: data,
             height: (height == null || height == undefined) ? 450 : "auto",
             editurl: 'clientArray',
             jsonReader : {  
                 root:"results",  
                 page: "page",  
             ...
             viewrecords: true,
             colModel:colModel,
             multiselect:(multiselect == null || multiselect == undefined || multiselect != false),
             scrollOffset : 0,
             shrinkToFit: false,
//             autowidth: true,
             autoScroll: true,
