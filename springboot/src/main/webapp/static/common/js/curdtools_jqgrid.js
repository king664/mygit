 ...
 * @param id//主键字段
 */
function update(title,url,gridId,width,height, obj) {
	// console.log("update", obj);
	if (obj && $(obj) && $(obj)[0].disabled) {
		return;
	}
 ...
 * @param id//主键字段
 */
function details(title,url, gridId,width,height) {
	var rowsData = $("#"+gridId).jqGrid('getGridParam','selarrrow');
	var multiselect=$("#"+gridId).jqGrid('getGridParam','multiselect');
	var rowData= $("#"+gridId).jqGrid('getGridParam','selrow');
	if(!multiselect)
    ...
    var id = rowsData[0];
    url=url.replace("{id}",id);
    openDialogView(title,url,width, height,"");
 ...
 * @return
 */
function toolbarSelectConfirm(title,url,gridId,tipMsg, obj) {
	if (obj && $(obj) && $(obj)[0].disabled) {
		return;
	}
 ...
 * @return
 */
function batchDelete(title,url,gridId, obj) {
	if (obj && $(obj) && $(obj)[0].disabled) {
		return;
	}
	var ids = [];
 ...
 * @param gridId 
 */  
function exports(title,url,gridId,width,height) {  
	var multiselect=$("#"+gridId).jqGrid('getGridParam','multiselect');
	 location.href=url;
	//openExports(title,url,width, height,"");
}  
 ...
 * @returns
 */
function imports(title,url,gridId,width,height){
    if (!width) {
    	width = "400px";
    }
    if (!height) {
	...
	openImportsView(title,url, width, height,"");
	
}
//打开对话框(查看)
function openImportsView(title, url, width, height) {
	...
	top.layer.open({
		...
		btn : [ '确定','关闭' ],
		yes : function(index) {
			var iframe1 = $(document).contents().find("iframe");
			if (iframe1 && iframe1[0] && iframe1[0].contentWindow) {
				var childWin = iframe1[0].contentWindow;
				if (childWin && childWin.user && childWin.user.uploadBtn) {
					childWin.user.uploadBtn();
					return;
				}
			}
		},
